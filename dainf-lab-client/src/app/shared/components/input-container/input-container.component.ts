...
export class InputContainerComponent {
  ...
  required = () => {
    ...
    const validators: ValidatorFn[] = (control.validator && (control.validator as any)?.validators) || [];
    if (control.validator && (control.validator as any).validators) {
      return (control.validator as any).validators.some((v: ValidatorFn) => v === Validators.required);
    }
  ...
  errorMessages = () => {
    ...
    if (errors['maxlength']) messages.push(`Máximo de ${errors['maxlength'].requiredLength} caracteres.`);
